...
class DashboardEduconecxAcademyController extends Controller
{
    public function index(Request $request, HeyGenLiveAvatarService $liveAvatarService): View
    {
        ...
        $contexts = $this->mergeFallbackContexts($liveAvatarService->listContexts());
    ...
    private function mergeFallbackContexts(array $contexts): array
    {
        $fallbackNames = ['Customer Support', 'Business Coach', 'AI Recruiter', 'Language Learning'];
        ...
        foreach ($fallbackNames as $name) {
            if (! $contextCollection->contains(fn ($context) => strcasecmp($context['name'], $name) === 0)) {
                $contextCollection->push([
                    'id' => '',
                    'name' => $name,
                ]);
        ...
        return $contextCollection->unique(fn ($context) => $context['id'] ?: $context['name'])->values()->all();
    }
