...
<style>
    ...
    .avatar-card, .context-card { border: 1px solid var(--academy-border); border-radius: 14px; padding: 14px; background: #fff; position: relative; }
    .avatar-card.selected, .context-card.selected { border: 2px solid var(--academy-navy); box-shadow: 0 0 0 4px rgba(251, 198, 12, .22); }
    .academy-avatar-preview { width: 100%; height: 170px; background: #f8f3dd; border-radius: 12px; overflow: hidden; display: flex; align-items: center; justify-content: center; margin-bottom: 12px; }
    .academy-avatar-img { width: 100%; height: 100%; object-fit: cover; display: block; }
    .academy-avatar-placeholder { font-size: 42px; color: var(--academy-navy); }
    ...
    @media (max-width: 1180px) { .avatar-grid { grid-template-columns: repeat(2, minmax(0, 1fr)); } .context-grid { grid-template-columns: repeat(2, minmax(0, 1fr)); } }
    ...
    @media (max-width: 560px) { .academy-dashboard { padding: 12px; } .academy-stats-grid, .avatar-grid, .context-grid { grid-template-columns: 1fr; } }
</style>
@endpush
        ...
        <main>
            ...
            @if(config('app.debug'))
                <details class="academy-api-debug">
                    ...
                    @php
                        $avatarDebugKeys = collect($liveAvatarDebug ?? [])
                            ->where('type', 'avatars')
                            ->pluck('first_raw_item_keys')
                            ->first(fn ($keys) => ! empty($keys)) ?? [];
                        $contextDebugKeys = collect($liveAvatarDebug ?? [])
                            ->where('type', 'contexts')
                            ->pluck('first_raw_item_keys')
                            ->first(fn ($keys) => ! empty($keys)) ?? [];
                    @endphp
                    <div class="row g-3 mt-2">
                        ...
                        <div class="col-md-6"><strong>First Raw Avatar Object Keys</strong><br>{{ $avatarDebugKeys ? implode(', ', $avatarDebugKeys) : 'No avatar object returned' }}</div>
                        <div class="col-md-6"><strong>First Raw Context Object Keys</strong><br>{{ $contextDebugKeys ? implode(', ', $contextDebugKeys) : 'No context object returned' }}</div>
                    </div>
                    <div class="mt-3">
            ...
            <section class="academy-card">
                <div class="academy-card-header">
                    <div>
                        ...
                        <p class="academy-card-subtitle">Selected public avatars are used on the Academy practice page, with env fallback only when LiveAvatar returns no avatars.</p>
                    </div>
                </div>
                                ...
                                <form method="POST" action="{{ route('dashboard.educonecx-academy.avatar-preference') }}" class="avatar-card {{ $selectedAvatar ? 'selected' : '' }}">
                                    ...
                                    <div class="academy-avatar-preview">
                                        @if(!empty($avatar['image_url']))
                                            <img src="{{ $avatar['image_url'] }}" alt="{{ $avatar['name'] }}" class="academy-avatar-img" loading="lazy">
                                        @else
                                            <i class="fas fa-user-astronaut academy-avatar-placeholder"></i>
                                        @endif
                                    </div>
            ...
            <section class="academy-card">
                <div class="academy-card-header">
                    <div>
                        <h2 class="academy-card-title">Choose LiveAvatar Context</h2>
                        <p class="academy-card-subtitle">Select the LiveAvatar context that will guide your speaking practice.</p>
                    </div>
                </div>
                <div class="academy-card-body">
                    @if(count($contexts))
