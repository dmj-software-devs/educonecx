        ...
        <main>
            ...
            <section class="academy-card">
                ...
                <div class="academy-card-body">
                        ...
                        <div class="avatar-grid">
                            @foreach($avatars as $avatar)
                                ...
                                <form method="POST" action="{{ route('dashboard.educonecx-academy.avatar-preference') }}" class="avatar-card {{ $selectedAvatar ? 'selected' : '' }}">
                                    ...
                                    @if(config('app.debug'))
                                        <details class="debug-id"><summary>Debug avatar_id</summary>{{ $avatar['id'] }}</details>
                                    @endif
                    ...
                    @else
                        <div class="alert alert-warning mb-0">No LiveAvatar avatars are available yet. Check the LiveAvatar API key or defaults.</div>
                    @endif
                </div>
            </section>
                ...
                <div class="academy-card-body">
                    ...
                    <div class="context-grid">
                        @foreach($contexts as $context)
                            ...
                            <form method="POST" action="{{ route('dashboard.educonecx-academy.scenario-preference') }}" class="context-card {{ $selectedContext ? 'selected' : '' }}">
                                @csrf
                                @if($selectedContext)<span class="selected-badge">Selected</span>@endif
                                <div class="context-name">{{ $context['name'] }}</div>
                                @if(config('app.debug') && !empty($context['id']))
                                    <details class="debug-id"><summary>Debug context_id</summary>{{ $context['id'] }}</details>
                                @endif
                                ...
                                <button type="submit" class="academy-btn-{{ $selectedContext ? 'soft' : 'navy' }} mt-2 w-100" {{ empty($context['id']) ? 'disabled' : '' }}>{{ $selectedContext ? 'Selected Context' : 'Select Context' }}</button>
                            </form>
                        @endforeach
                    </div>
                ...
                <div class="academy-card-body">
                    <div class="academy-table-wrap">
                        <table class="academy-table">
                            <thead>
                                <tr>
                                    <th>Date</th><th>Scenario</th><th>Category</th><th>Avatar</th><th>Overall</th><th>Pron.</th><th>Grammar</th><th>Fluency</th><th>Vocabulary</th><th>Status</th><th>Action</th>
                                </tr>
                            </thead>
                            <tbody>
                                ...
                                @empty
                                    <tr><td colspan="11" class="text-center py-4">No speaking practice yet. Start your first AI avatar practice.</td></tr>
                                @endforelse
                            </tbody>
                        </table>
                    </div>
