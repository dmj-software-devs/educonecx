...
.learning-grid { display:grid; grid-template-columns:minmax(0, 1fr) minmax(300px, 360px); gap:30px; align-items:start; }
.learning-video-section { min-width:0; max-width:100%; }
...
.lesson-content h2 { color: var(--prussian-blue); margin-bottom: 20px; font-size: 1.5rem; padding: 14px 18px; border-left: 5px solid var(--bright-amber); background: linear-gradient(90deg, rgba(251,198,12,.16), transparent); border-radius: var(--radius-md); }
...
/* ============================================================
   SIDEBAR
============================================================ */
.learning-sidebar { position: sticky; top: 120px; align-self: start; min-width: 0; z-index: 2; }
.curriculum-container { background: var(--pure-white); border-radius: var(--radius-lg); overflow: hidden auto; box-shadow: var(--shadow-md); border: 1px solid rgba(251,198,12,.1); max-height: calc(100vh - 150px); display: flex; flex-direction: column; }
...
@media (max-width: 1200px) {
    .learning-grid { grid-template-columns: minmax(0, 1fr) minmax(280px, 330px); gap: 24px; }
}
@media (max-width: 992px) {
    .learning-grid { grid-template-columns: 1fr; }
    .learning-sidebar { position: static; margin-top: 30px; }
    .curriculum-container { max-width: 760px; max-height: none; margin: 0 auto; }
}
...
@media (max-width: 576px) {
    ...
    .lesson-nav-center { display: flex; order: -1; flex-basis: 100%; margin-bottom: 6px; }
    .lesson-nav-title { white-space: normal; display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical; font-size: .86rem; }
    .vp-title-bar { opacity: 1; transform: none; padding: 10px 12px 32px; }
    .vp-title-bar-text { white-space: normal; display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical; font-size: .78rem; }
    .vp-controls { padding: 0 10px 10px; }
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function buildEmbedCandidates(url = '') {
        ...
        const out = [withAutoplayParam(normalized)];
